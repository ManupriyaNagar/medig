  ...
  return (
    <header className="sticky top-0 z-50 bg-white shadow-md">
      ...
      <div className="bg-green-600 text-white text-center py-1 text-xs">
        <p>Free delivery on orders above ₹999 | Use code MEDI100 for 10% off on your first order</p>
      </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center">
            <Link href="/" className="text-2xl font-bold text-green-600 flex items-center">
              <Image src="/nav.png" alt="MediGlucks" width={100} height={60} className="mr-2" />
            </Link>
          </div>
          ...
          <div className="flex items-center space-x-4">
            ...
            <button className="flex items-center text-sm font-medium hover:text-green-600">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="container mx-auto px-4">
          <ul className="flex space-x-8">
            {navData.menu.slice(0, 6).map((item, index) => (
              ...
              >
                <button className="text-sm font-medium hover:text-green-600 flex items-center">
                  {item.name}
                ...
                {activeCategory === index && (
                  <div className="absolute left-0 mt-2 w-60 bg-white shadow-lg rounded-md overflow-hidden z-50">
                    <div className="p-4">
                      {item.subcategories.slice(0, 8).map((subcat, subIndex) => (
                        <div key={subIndex} className="mb-2">
                          {typeof subcat === 'string' ? (
                            ...
                            <div>
                              <Link href="#" className="block text-sm font-medium hover:text-green-600">
                                {subcat.name}
                              </Link>
                              <div className="ml-3 mt-1">
                                {subcat.items && subcat.items.slice(0, 4).map((item, itemIndex) => (
                                  <Link href="#" key={itemIndex} className="block text-xs text-gray-600 hover:text-green-600 mt-1">
                                    {item}
                                  </Link>
                                ))}
                              </div>
            ...
            <ul className="space-y-4">
              {navData.menu.slice(0, 6).map((item, index) => (
                <li key={index}>
                  ...
                  >
                    {item.name}
                    <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 transition-transform ${activeCategory === index ? 'transform rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                    </svg>
                  </button>
